...
/* Global variables                                                       */
/**************************************************************************/
//(-/+) 210610 CJKIM, volatile keyword 
static volatile mss_mac_instance_t g_mac;
...
//			cfg ->mac_addr[0], cfg ->mac_addr[1], cfg ->mac_addr[2], cfg ->mac_addr[3], cfg ->mac_addr[4], cfg ->mac_addr[5]);
	...
	printf("low_level_init/MSS_MAC_init  --> network auto negotiate ...\r\n");
#if (MSS_MAC_PHY_INTERFACE != BASEX1000)
...
)
{
	...
	//210621 CJKIM , tcp_server_send() 에서 disable 처리.
    //NVIC_DisableIRQ(EthernetMAC_IRQn);
	...
	if(g_mac.nb_available_tx_desc > 0)
	{
		...
		p_next_tx_desc = (mss_mac_tx_desc_t *)&g_mac.tx_desc_tab[g_mac.next_tx_index];
	...
	//210621 CJKIM , tcp_server_send() 에서 enable 처리.
    //NVIC_EnableIRQ(EthernetMAC_IRQn);
...
)
{
    ...
    if(packet_sent != 0u)
    {
		 ...
		 */
		__ASM volatile ("dsb"); //(-/+) 210611 CJKIM, added for network test
    }
...
)
{
    ...
    do
    {
        mss_mac_rx_desc_t * cdesc = (mss_mac_rx_desc_t *)&g_mac.rx_desc_tab[g_mac.first_rx_desc_index];
...
static void tx_desc_ring_init(void)
{
    ...
    for(inc = 0; inc < MSS_MAC_TX_RING_SIZE; ++inc)
    {
        ...
        {
            g_mac.tx_desc_tab[inc].next_desriptor = (mss_mac_tx_desc_t *)&g_mac.tx_desc_tab[0];
        }
        else
        {
            g_mac.tx_desc_tab[inc].next_desriptor = (mss_mac_tx_desc_t *)&g_mac.tx_desc_tab[inc + 1];
        }
...
static void rx_desc_ring_init(void)
{
    ...
    for(inc = 0; inc < MSS_MAC_RX_RING_SIZE; ++inc)
    {
        ...
        {
            g_mac.rx_desc_tab[inc].next_desriptor = (mss_mac_rx_desc_t *)&g_mac.rx_desc_tab[0];
        }
        else
        {
            g_mac.rx_desc_tab[inc].next_desriptor = (mss_mac_rx_desc_t *)&g_mac.rx_desc_tab[inc + 1];
        }
